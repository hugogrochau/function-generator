...
void * gera_func(void *f, int n, Parametro params[]) {
    ...
    void *nova_func = malloc(NUM_BYTES_INICIALIZACAO + tam_bytes_params + NUM_BYTES_CALL + NUM_BYTES_FINALIZACAO);
...
void gera_instr_call(int n, Parametro params[], ByteArray bytes_func) {
    int i, posicao;
    size_t num_bytes = 0;
    for (i = 0; i < n; i++) { /* parametros */
    if (params[i].amarrado) {
        switch (params[i].tipo) {
            case DOUBLE_PAR:
            *(bytes_func++) = OP_PUSH_LONG;
            *((long *)bytes_func) = (long) params[i].valor.v_double << (sizeof(double)/2);
        ...
        switch (params[i].tipo) {
            ...
            *((short *) bytes_func) = OP_PUSH_EBP;
            bytes_func += 2;
            *(bytes_func++) = (Byte) posicao + sizeof(double)/2;
            *((short *) bytes_func) = OP_PUSH_EBP;
            bytes_func += 2;
